...
# 외부 조인(outer join)
# : 두 개 이상의 테이블을 연결 할때, 조인 조건에 맞지 않는 행도 포함하여 결과를 반환
...
# 3. full join
# : 양쪽 테이블의 모든 행 포함, 일치하지 않으면 null로 채움
...
# 각 고객의 이르뫄 새당 고객이 주문한 총 주문 건수과 주문 날짜를 조회
# 주문이 없는 고객도 포함하여 조회, 주문 건수는 0으로 표시

select
	C.name
    , count(O.order_id) as totla_orders
